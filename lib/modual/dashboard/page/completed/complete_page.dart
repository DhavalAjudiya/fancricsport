  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: AppColor.blackDarkT,
        title: const AppText("Completed Match",
            color: AppColor.white, fontWeight: FontWeight.w600),
      ),
      body: SafeArea(
        child: Obx(
          () => (matchController.completedList.value?.allMatch?.isEmpty ?? false)
              ? const Center(
              ...
              : ListView.separated(
                  ...
                  shrinkWrap: true,
                  itemCount: matchController.completedList.value?.allMatch?.length ?? 0,
                  itemBuilder: (context, index) {
                    final data = matchController.completedList.value?.allMatch?[index];
                    ...
                    return Padding(
                      ...
                      child: completedCard(
                          onTap: () async {
                            ...
                            matchController.completedImage.value = data?.imageUrl ?? "";
                            await matchController
                                .getAllPlayer(matchController.completedMatchId.value);
                            Navigation.pushNamed(Routes.completedPageBottomPage);

                            await matchController
                                .matchRunData(matchController.completedMatchId.value);
                            await matchController
                                .matchStatus(matchController.completedMatchId.value);
                          },
                          toss: "",
                          result: data?.result,
                          showDate: timeHeader,
